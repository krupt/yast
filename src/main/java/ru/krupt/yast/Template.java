...
public class Template {
    ...
    private Iterable<Part> parseTemplate(String template) throws InvalidParameterFormat {
        ...
        while (parameterFinder.find()) {
            ...
            int parameterNo = Integer.parseInt(formatMatcher.group(2)) - 1;
    ...
    public String render(List<String> parameters) throws ParameterIsNullAndNotHaveDefaultValueException {
        ...
        for (Part part : parts) {
            ...
            OptionalInt parameterNo = part.getParameterNo();
            if (parameterNo.isPresent()) {
                String parameterValue = null;
                try {
                    parameterValue = parameters.get(parameterNo.getAsInt());
                ...
                if (parameterValue != null) {
                    ...
                    if (part.isDefaultValueExists()) {
                        builder.append(part.getDefaultValue());
                    } else {
                        throw new ParameterIsNullAndNotHaveDefaultValueException(parameterNo.getAsInt() + 1);
                    }
                }
            }
        }
